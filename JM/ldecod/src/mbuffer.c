...
struct track   //top left amd right bottom pixel of the box we draw
{
	int spx;
	int spy;
	int epx;
...
void rec_box(int x,int y,frame_mv *fmv,imgpel **Y,struct list_type3 * list,int MVthresh,int row_length)
{
	...
	for(b=0;b<4;b++)
			 for(c=0;c<4;c++)
				 if(((fmv[i].mv[0][b][c][0]*fmv[i].mv[0][b][c][0]+fmv[i].mv[0][b][c][1]*fmv[i].mv[0][b][c][1]>=MVthresh*MVthresh)||(fmv[i].mv[1][b][c][0]*fmv[i].mv[1][b][c][0]+fmv[i].mv[1][b][c][1]*fmv[i].mv[1][b][c][1]>=MVthresh*MVthresh))&&(visited[b][c]==0))
				{
...
struct track plot_big_box(imgpel **Y, struct list_type3 * list, double distTH)
{
	int rootcount=0;
	int lone;
	int i,j;
	...
	rec_tracking.epx=epx;
	rec_tracking.epy=epy;
	rec_tracking.spx=spx;
	rec_tracking.spy=spy;
	return rec_tracking;
}	
void track_box(imgpel **Y,struct track rec_tracking)
{
	int i,j;
	if((rec_tracking.epx!=0)||(rec_tracking.epy!=0)||(rec_tracking.spx!=0)||(rec_tracking.spy!=0))
	{
	for(j=rec_tracking.spy;j<=rec_tracking.epy;j++)
	{
		if((j==rec_tracking.spy)||(j==rec_tracking.epy))
			for(i=rec_tracking.spx;i<=rec_tracking.epx;i++)
			{
				Y[j][i]=0;
			}
		else
...
void sri_compute_thresh(frame_mv *filemv,int mbs)
{
	int i,b,c,a,t,mean,var;double sita=0.0;int r1,r2;
	int sum=0;int norm;int count;int resolution=0;
	count=0;
	mean=0;
	var=0;
	for(i=0;i<mbs;i++)
	{
		...
		for(b=0;b<4;b++)
			for(c=0;c<4;c++)
				if((norm=sqrt((double)(filemv[i].mv[a][b][c][0]*filemv[i].mv[a][b][c][0]+filemv[i].mv[a][b][c][1]*filemv[i].mv[a][b][c][1])))>=1)
				{
					sum+=norm;
					count++;
				}
	...
	var=var/count;
	sita=sqrt((double)var);
	resolution=6*((double)sita)/10;
	}
	r1=mean-3*sita;
	r2=mean+3*sita;
	printf("\mean range1 range2 nbiaozhuncha fangcha count threshold %d %d %d %d %f %d %d\n",mean,r1,r2,var,sita,count,resolution);
}
int compute_thresh(frame_mv *filemv,int mbs)
{
	...
	for(i=0;i<mbs;i++)
	{
		...
		for(b=0;b<4;b++)
			for(c=0;c<4;c++)
              if((norm=sqrt((double)(filemv[i].mv[a][b][c][0]*filemv[i].mv[a][b][c][0]+filemv[i].mv[a][b][c][1]*filemv[i].mv[a][b][c][1])))>=5)
				{
				  t=(int)(norm/5);
				  mm[(t>=10)?9:t-1]++;
				}
	...
	if(max!=0)
		...
		max=5;
 printf("%d %d %d %d %d %d %d %d %d %d ",mm[0],mm[1],mm[2],mm[3],mm[4],mm[5],mm[6],mm[7],mm[8],mm[9]);
 printf("\nwhat a max %d\n",max);
 return max;
}
int search_nonzero(int x,int y,frame_mv *fmv,int MVthresh,int row_length)
{
     ...
     for(b=0;b<4;b++)
	{
			 for(c=0;c<4;c++)
				 ...
				 if((fmv[i].mv[0][b][c][0]*fmv[i].mv[0][b][c][0]+fmv[i].mv[0][b][c][1]*fmv[i].mv[0][b][c][1]>=MVthresh)||(fmv[i].mv[1][b][c][0]*fmv[i].mv[1][b][c][0]+fmv[i].mv[1][b][c][1]*fmv[i].mv[1][b][c][1]>=MVthresh*MVthresh))
				   {
					   f=1;
					   break;
				 }
...
//sri_compute_thresh(filemv,p_Vid->PicSizeInMbs);
...
if(list->length>0)
	tracking=plot_big_box(p_Dpb->fs[pos]->frame->imgY,list, distTH);  //plot the big box and record the pixel information of the box
else
	track_box(p_Dpb->fs[pos]->frame->imgY,tracking);  //do tracking when there is no motion detected
free(filemv);
filemv=NULL;
